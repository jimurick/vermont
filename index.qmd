...
3. Open the project.
    - Open RStudio and click File → Open Project...
    - In the file dialog, go in the `vermont` repo folder you just created, and open the file `'vermont.Rproj'`
    - In the RStudio Console window, run the R function `renv::restore()`
4. (Optional) If you want to run the Shiny app, you'll need to install my `hospitalstars` package first.
    - In the RStudio Console, run the R command `renv::install("devtools")`.
